...
| `/about`, `/now` | GET, HEAD | `standalone-page` | One route generated per `db/page-slugs` entry. `/about` renders the full visible h-card; everything else renders as a plain page. |
| `/feed` | GET, HEAD | `rss-feed` | RSS of articles only — responses and pages are excluded. Carries the WebSub `Link` headers (`rel="hub"`, `rel="self"`). |
